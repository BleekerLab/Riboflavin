...
p7 <- relative_flies %>%
  ...
  geom_smooth(method = drm, method.args = list(fct = L.5()), 
              se = FALSE) + 
   geom_smooth(aes(x = day,
                y = median_number, color = genotype), method = "glm", method.args = list(family = "binomial"), formula = y ~ splines::bs(x), se = FALSE)
  scale_color_manual(values = palette) +
  theme_simple()
...
Below are graphs showing the derivative of the cumulative data. I wanted to show the counts per day and not the growh speed of the cumulative curve, but at least it's already the type of model I could use. Later more
