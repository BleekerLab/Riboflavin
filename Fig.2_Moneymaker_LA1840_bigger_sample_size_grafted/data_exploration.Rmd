...
Trying to figure out how to get the statistics:
```{r}
# define drm function to use with map
drm.func <- function(x) {
  drm(first_instars ~ day, 
      fct = W1.3(names = c("Slope", "Upper Limit", "ED50")), 
      data = x)
}

predict.fun <- function(x) {
  add_predictions(data.frame(day = seq(0,15)), x)
}

coefs.fun <- function(x) {coef(x) %>% tibble()}


df2 <- filter(flies_model, day<15) %>% group_by(genotype) %>% nest() %>%
...
# define drm function to use with map
drm.func <- function(x) {
  drm(percentage_from_hatched ~ day, 
      fct = lgaussian(), 
      data = x)
}
...
df2 <- flies_first %>% group_by(genotype) %>% nest() %>%
  mutate(drmod = map(data, drm.func), 
         pred = map(drmod, predict.fun),
         coefs = map(drmod, coefs.fun))
