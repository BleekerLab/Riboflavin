...
flies_cv <- flies %>%
  ...
  dplyr::filter(day < 18)  %>%
  dplyr::filter(id != "cv_rib_4")  %>%
  dplyr::filter(id != "cv_rib_5")  %>%
  dplyr::filter(id != "cv_c_3") 

# cv_rib_4 and cv_rib_5 are taken out, because the leaves died 
# cv_c_3 is taken out, because it did not grow roots while all other control leaves did

# The first 10 rows of the data
knitr::kable(flies_cv[1:10,1:8])
...
```{r cumulative data}
cumulative_flies_cv <- flies_cv 

cumulative_flies_cv <- cumulative_flies_cv %>%
  mutate(first_instars = rowSums(cumulative_flies_cv[,8:11])) %>%
  mutate(second_instars = rowSums(cumulative_flies_cv[,9:11])) %>%
  mutate(third_instars = rowSums(cumulative_flies_cv[,10:11])) %>%
...
cumulative_flies_cv <- cumulative_flies_cv %>%
  ...
  rel_cumul = tibble()
 for(i in c("cv_c_1", "cv_c_2", "cv_c_4", "cv_c_5", "cv_rib_1", "cv_rib_2", "cv_rib_3")){
   subdata = subset(cumulative_flies_cv, id == i)
...
cumulative_flies_cv <- rel_cumul %>%
  unite("first_instars", cv_c_1:cv_rib_3, 
                                  na.rm = TRUE)  %>%
...
cumulative_flies_cv <- rel_cumul %>%
  unite("second_instars", cv_c_1:cv_rib_3, 
                                  na.rm = TRUE)  %>%
...
cumulative_flies_cv <- rel_cumul %>%
  unite("third_instars", cv_c_1:cv_rib_3, 
                                  na.rm = TRUE)  %>%
...
cumulative_flies_cv <- rel_cumul %>%
  unite("fourth_instars", cv_c_1:cv_rib_3, 
                                  na.rm = TRUE)
cumulative_flies_cv[cumulative_flies_cv == ""]<-NA
```
...
```{r glm}
df_glm_eggs <- cumulative_flies_cv %>%
                dplyr::filter(day == 17) %>%
                dplyr::filter(stage == "eggs_start")

df_glm_3rd <- cumulative_flies_cv %>%
                dplyr::filter(day == 17) %>%
                dplyr::filter(stage == "third_instars")



glm_eggs <- glm(formula = number ~ treatment,
            family = poisson(),
            data = df_glm_eggs)

glm_third_instars <- glm(formula = number ~ treatment,
                      family = poisson(),
                      data = df_glm_3rd)

summary(glm_eggs)
summary(glm_third_instars)
```

```{r t-test}
df_ttest_third <- cumulative_flies_cv %>%
  dplyr::filter(day == 17) %>%
  dplyr::filter(stage == "third")

t.test(formula = number ~ treatment, 
       data = df_ttest_third)
```
...
cumulative_flies_cv %>%
  dplyr::filter(day == 17) %>%
  dplyr::filter(stage == "eggs_start" | stage == "third_instars" | stage == "third") %>%
  group_by(stage) %>%
  wilcox_test(formula = number ~ treatment)
```
...
cumulative_flies_cv %>%
  ...
  dplyr::filter(stage == "eggs_start" | stage == "third_instars" | stage == "third") %>%
  group_by(stage) %>%
  cohens_d(formula = number ~ treatment)
...
p_eggs <- cumulative_flies_cv %>% 
  ...
  theme_simple() + 
  scale_fill_manual(values = cv_palette)  +
  geom_signif(comparison = list(c("control","riboflavin")), 
              map_signif_level = TRUE, 
              annotations = c("ns"),
              y_position = 148.57)


p_third <- cumulative_flies_cv %>% 
  dplyr::filter(day == 17) %>%
  dplyr::filter(stage == "third_instars") %>%
  ggplot(aes(x=treatment, y=number, fill=treatment)) + 
...
collected <- (p_eggs | p_third | p_3)  / guide_area() +
...
flies_LA <- flies_LA %>%
  ...
  dplyr::mutate(`4_fourth_instar` = `4_early_fourth_instar` + `5_completed_lifecycle`) %>%
  dplyr::ungroup()
```
...
```{r cumulative data}
cumulative_flies_LA <- flies_LA 

cumulative_flies_LA <- cumulative_flies_LA %>%
  mutate(first_instars = rowSums(cumulative_flies_LA[,7:10])) %>%
  mutate(second_instars = rowSums(cumulative_flies_LA[,8:10])) %>%
  mutate(third_instars = rowSums(cumulative_flies_LA[,9:10])) %>%
  mutate(fourth_instars = `4_fourth_instar`)
```
...
cumulative_flies_LA <- cumulative_flies_LA %>%
 ...
 for(i in c("LA1840_c_1", "LA1840_c_2", "LA1840_c_3", "LA1840_c_4", "LA1840_c_5", "LA1840_in_1", "LA1840_in_2", "LA1840_in_3", "LA1840_in_4", "LA1840_in_5")){
   subdata = subset(cumulative_flies_LA, id == i)
   subdata[i] = cumul_median(subdata)
   rel_cumul <- bind_rows(rel_cumul, subdata)
 }


cumulative_flies_LA <- rel_cumul %>%
...
cumulative_flies_LA <- cumulative_flies_LA %>%
  dplyr::group_by(id) %>%
  dplyr::mutate(eggs_start = as.numeric(as.character(max(`0_egg`, na.rm = TRUE)))) %>%
  dplyr::ungroup()
...
cumulative_flies_LA <- cumulative_flies_LA %>%
  ...
  dplyr::mutate(second = (second_instars / eggs_start)*100)%>%
  dplyr::mutate(third = (third_instars / eggs_start)*100)%>%
  dplyr::mutate(fourth = (fourth_instars / eggs_start)*100) %>%
  dplyr::ungroup() 

cumulative_flies_LA <- cumulative_flies_LA %>%
  pivot_longer(cols = fourth_instars:fourth, 
               names_to = "stage", values_to = "number") %>%
  mutate(treatment = factor(treatment, levels = c("control", "inhibitor"))) %>%
  mutate(stage = factor(stage, levels = c("eggs_start", "first_instars", "second_instars", "third_instars", "fourth_instars", "first", "second", "third", "fourth"))) 

auc <- cumulative_flies_LA
```
...
```{r glm}
df_glm_eggs <- cumulative_flies_LA %>%
                dplyr::filter(day == 22) %>%
                dplyr::filter(stage == "eggs_start")

df_glm_3rd <- cumulative_flies_LA %>%
                dplyr::filter(day == 22) %>%
                dplyr::filter(stage == "third_instars")
...
```{r t-test}
df_ttest_third <- cumulative_flies_LA %>%
  dplyr::filter(day == 22) %>%
  dplyr::filter(stage == "third")
...
```{r calculate median eggs to 3rd (%)}
cumulative_flies_LA %>%
  dplyr::filter(day == 22) %>%
  dplyr::filter(stage == "third") %>%
  group_by(treatment) %>%
...
##LA1840 and combined plots
```{r combined plot, fig.width = 16, fig.height = 10}

p_eggs_i <- cumulative_flies_LA %>% 
  dplyr::filter(day == 2) %>%
  dplyr::filter(stage == "eggs_start") %>%
  ggplot(aes(x=treatment, y=number, fill=treatment)) + 
  labs(y = "eggs (count)") +
  ...
  geom_signif(comparison = list(c("control","inhibitor")), 
              map_signif_level = TRUE, 
              annotations = c("ns"),
              y_position = 148.57) 

p_third_i <- cumulative_flies_LA %>% 
  dplyr::filter(day == 22) %>%
  dplyr::filter(stage == "third_instars") %>%
  ggplot(aes(x=treatment, y=number, fill=treatment)) + 
  labs(y = "3rd instar nymphs (count)") +
...
collected <- ((p_eggs | p_third| p_3) / (p_eggs_i | p_third_i | p_3_i))  /  guide_area() +
  plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = 'a')

collected

ggsave(filename = "hydroponics_bioassays_boxplots.png", plot = collected, width = 10, height = 10)

ggsave(filename = "hydroponics_bioassays_boxplots.svg", plot = collected, width = 10, height = 10)
```

#############################################################################

```{r median and IQR}
relative_cumulative <- cumulative_flies_LA %>%
...
cumulative_flies_LA <- cumulative_flies_LA %>% mutate(day = as.numeric(as.character(day)))
...
#              linetype = "dotted") +
  ...
  geom_line(aes(x = day,
                ...
                color = treatment),
            size = 1) +
    stat_compare_means(data = cumulative_flies_LA %>% dplyr::filter(stage == "second"), aes(x = day, 
               y = number, 
               group = treatment), 
...
#              linetype = "dotted") +
  ...
  geom_line(aes(x = day,
                ...
                color = treatment),
            size = 1) +
  stat_compare_means(data = cumulative_flies_LA %>% dplyr::filter(stage == "third") %>% dplyr::filter(day == 22), aes(x = day,
               y = number, 
               group = treatment), 
               label.y = 62, label = "p.signif", method = "t.test") +
...
#              linetype = "dotted") +
  ...
  geom_line(aes(x = day,
                ...
                color = treatment),
            size = 1) +
    stat_compare_means(data = cumulative_flies_LA %>% dplyr::filter(stage == "fourth"), aes(x = day,
               y = number, 
               group = treatment), 
               label.y = 65, label = "p.signif", method = "t.test") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 70))
...
#ggsave(filename = "inhibitor_bioassay_lineplots_1.png", plot = p_cumulative, width = 10, height = 5)
```

```{r auc, fig.width = 16, fig.height = 10}
library(DescTools)
...
auc_fourth <- auc %>%
  ...
  auc_fourth <- rel_cumul %>%
    unite("auc", LA1840_c_1:LA1840_in_5, 
          na.rm = TRUE) %>%  
  dplyr::filter(day == 14) %>% 
    mutate(auc = as.numeric(as.character(auc)))
...
development_plot <- (p_third + inset_element(auc_plot, 
                             left = 0.57, 
                             bottom = 0.025, 
                             right = 1, 
                             top = 0.55)) + guide_area() +
...
ggsave(filename = "inhibitor_lineplots_3rd.svg", plot = development_plot, width = 15, height = 10)
```
