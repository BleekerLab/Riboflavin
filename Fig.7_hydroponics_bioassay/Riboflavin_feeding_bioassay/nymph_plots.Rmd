...
#(p_cv| p_LA1840) / (p_2 | p_3 | p_4)) / (p_M | p_L)) /

ggsave(filename = "riboflavin_assay_boxplots.png", plot = collected, width = 10, height = 10)

ggsave(filename = "riboflavin_assay_boxplots.svg", plot = collected, width = 10, height = 10)
```
...
relative_cumulative <- relative_cumulative %>%
  ...
  mutate(median_abundance = median(
    c_across(cv_c_1:cv_rib_5), na.rm = TRUE)) %>%
...
#  dplyr::filter(day > 9) %>% 
  ...
  scale_color_manual(values = cv_palette) +
  scale_fill_manual(values = cv_palette) +
#  geom_line(aes(x = day,
#                  y = min_abundance,
#                  color = genotype),
...
#  dplyr::filter(day > 9) %>% 
  ...
  ggplot() +
  theme_big_simple() +
  labs(y = "development to 3rd instar instar (%)",
       x = "days after infection") +
  scale_color_manual(values = cv_palette) +
  scale_fill_manual(values = cv_palette) +
#  geom_line(aes(x = day,
...
#              linetype = "dotted") +
  ...
  geom_line(aes(x = day,
                ...
                color = treatment),
            size = 1) +
    stat_compare_means(data = cumulative_flies %>% dplyr::filter(stage == "third") %>% dplyr::filter(day == 17), aes(x = day,
               y = number, 
               group = treatment), 
...
auc_fourth <- auc %>%
  ...
  rel_cumul = tibble()
  for(i in c("cv_c_1", "cv_c_2", "cv_c_3", "cv_c_4", "cv_c_5", "cv_rib_1", "cv_rib_2", "cv_rib_3")){
...
ggsave(filename = "hydroponics_lineplots_3rd.svg", plot = development_plot, width = 15, height = 10)
```
