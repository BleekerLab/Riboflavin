---
title: "bioassay on grafts without leaves on the rootstocks"
...
cumul_median <- function(data) {
    ...
    if(data$day[i] == 6) { data$var_new[i] <- data$var_new[i]  
    } else {
...
# The raw data
```{r flies}
flies <- read.csv("bioassay_grafts_no_leaves.csv", 
                         header = T, 
                         sep = ",", 
                         dec = ".", 
                         check.names = FALSE) %>%
...
relative_cummulative <- relative_cummulative %>%
  dplyr::mutate(first_instars = rowSums(relative_cummulative[,5:8])) %>%
  dplyr::mutate(second_instars = rowSums(relative_cummulative[,6:8])) %>%
  dplyr::mutate(third_instars = rowSums(relative_cummulative[,7:8])) %>%
...
relative_cummulative <- relative_cummulative %>%
  ...
  rel_cumul = tibble()
 for(i in c("LA1840_2", "LA1840_3", "LA1840_4", "LA1840_5", "cv_1", "cv_2", "cv_3", "cv_4", "cv_5")){
...
relative_cummulative <- rel_cumul %>%
  unite("first_instars", LA1840_2:cv_5, 
                                  na.rm = TRUE)  %>%
...
relative_cummulative <- rel_cumul %>%
  unite("second_instars", LA1840_2:cv_5, 
                                  na.rm = TRUE)  %>%
...
relative_cummulative <- rel_cumul %>%
  unite("third_instars", LA1840_2:cv_5, 
                                  na.rm = TRUE)  %>%
...
relative_cummulative <- rel_cumul %>%
  unite("fourth_instars", LA1840_2:cv_5, 
                                  na.rm = TRUE)
```
...
relative_cummulative <- relative_cummulative %>%
  dplyr::group_by(id) %>%
  dplyr::mutate(eggs_start = max(`0_egg`))%>%
...
relative_cummulative <- relative_cummulative %>%
  pivot_longer(cols = first_instars:fourth, 
...
ed_values <- tibble::tribble(
  ~genotype, ~ed,   ~stat,   ~value,
    "cv",   "upper limit  ", "mean (±SE)",  "85.29 (±2.92)",
    "cv",   "slope", "mean (±SE)",  "14.75 (±4.73)",
    "cv",   "EmT50", "mean (±SE)",  "21.00 (±0.32)",
    "LA1840",   "upper limit  ", "mean (±SE)",  "85.82 (±3.80)",
    "LA1840",   "slope", "mean (±SE)",  "12.02 (±3.12)",
    "LA1840",   "EmT50", "mean (±SE)",  "21.93 (±0.40)",
    "cv",   "upper limit  ", "x",  " ",
    "cv",   "slope", "x",  " ",
    "cv",   "EmT50", "x",  " ",
...
relative_cummulative <- relative_cummulative %>%
  dplyr::mutate(day = as.factor(day)) %>%
  rowwise() %>%
  dplyr::mutate(median_abundance = median(c_across(`LA1840_2`:`cv_5`), na.rm = TRUE)) %>%
  dplyr::mutate(max_abundance = max(c_across(`LA1840_2`:`cv_5`), na.rm = TRUE)) %>%
  dplyr::mutate(min_abundance = min(c_across(`LA1840_2`:`cv_5`), na.rm = TRUE)) %>%
  dplyr::mutate(IQR_abundance = IQR(c_across(`LA1840_2`:`cv_5`), na.rm = TRUE)) %>%
  dplyr::mutate(day = as.numeric(as.character(day))) %>%
  dplyr::select(-`LA1840_2`:-`cv_5`) 
...
relative_abundance <- relative_abundance %>%
  ...
  pivot_wider(names_from = id, values_from = relat_abund) 
