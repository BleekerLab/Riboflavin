---
title: "qPCR on root and shoot - riboflavin synthase"
...
The expression of riboflavin synthase in root and shoot of LA1840, MB, IL27 and IL28 normalised to actin. Normalisation by Petra Bleeker, data from Julia Zwaan. 

## The raw data
```{r flies}
flies <- read.csv("leaf_root_cleaned_Petra.csv", 
                         header = T, 
                         sep = ",", 
                         dec = ".", 
                         check.names = FALSE) %>%    
  ...
  mutate_if(is.character, str_replace_all, 
            pattern = "IL1927", replacement = "IL27") %>%    
  mutate_if(is.character, str_replace_all, 
            pattern = "IL1928", replacement = "IL28")
...
flies <- flies %>%
  ...
  pivot_wider(names_from = tissue, values_from = normalised_ribo_synth_expr) 
```
 

```{r}
flies <- flies %>%
  mutate(genotype = factor(genotype, levels = c("cv", "IL28", "IL27", "LA1840"))) 
print(flies)
```
...
palette2 <- c("#E69F00", "#0072B2", "#56B4E9")
```
...
p1 <- flies %>% 
  ...
  labs(y = "normalised riboflavin synthase expression (leaves)") +
...
p2 <- flies  %>%
  ...
  ggplot(aes(x=genotype, y=root)) + 
  labs(y = "normalised riboflavin synthase expression (root)") +
  geom_boxplot(aes(fill=genotype)) +   
  geom_point(aes(fill = genotype), size = 2, shape = 21,
             position = position_jitterdodge(jitter.width = 0.4)) + 
  theme_simple() + 
...
ggsave(filename = "ribo_synth_expression.png", plot = collected, width = 10, height = 10)
ggsave(filename = "ribo_synth_expression.svg", plot = collected, width = 10, height = 10)
```
